...
func TestPacketV2Signature(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			[]byte{0xfd, 0x2, 0x1, 0x0, 0x0, 0x0, 0x0, 0xde, 0x0, 0x0, 0xf, 0x27, 0x96, 0x90, 0xe, 0x47, 0x4, 0xc, 0xef, 0x3b, 0xe, 0x47, 0x4, 0xc, 0xef, 0xef, 0x9b},
			signature.Signature{0xe, 0x47, 0x4, 0xc, 0xef, 0x3b, 0xe, 0x47, 0x4, 0xc, 0xef, 0xef, 0x9b},
		},
	}
	for i, c := range cases {
...
func TestPacketCopy(t *testing.T) {
	...
	cases := []packet.Packet{
		...
		&packet2{
			...
			payload:       payload,
			incompatFlags: 0x01,
			signature:     signature.NewSignature(0xAA, time.Now(), [6]byte{0x01, 0x02, 0x03, 0x04, 0x05, 0x06}),
		},
	}
