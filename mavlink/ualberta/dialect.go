...
func (m *NavFilterBias) Unmarshal(payload []byte) error {
...
type RadioCalibration struct {
	Aileron  []uint16 `len:"3" ` // Aileron setpoints: left, center, right
	Elevator []uint16 `len:"3" ` // Elevator setpoints: nose down, center, nose up
	Rudder   []uint16 `len:"3" ` // Rudder setpoints: nose left, center, nose right
	Gyro     []uint16 `len:"2" ` // Tail gyro mode/gain setpoints: heading hold, rate mode
	Pitch    []uint16 `len:"5" ` // Pitch curve setpoints (every 25%)
	Throttle []uint16 `len:"5" ` // Throttle curve setpoints (every 25%)
}
...
// Unmarshal (generated function)
func (m *RadioCalibration) Unmarshal(payload []byte) error {
...
// Unmarshal (generated function)
func (m *UalbertaSysStatus) Unmarshal(payload []byte) error {
...
func init() {
	mavlink.Register(MSG_ID_NAV_FILTER_BIAS, "MSG_ID_NAV_FILTER_BIAS", 32, 34, func(p mavlink.Packet) (mavlink.Message, error) {
	...
	mavlink.Register(MSG_ID_RADIO_CALIBRATION, "MSG_ID_RADIO_CALIBRATION", 42, 71, func(p mavlink.Packet) (mavlink.Message, error) {
	...
	mavlink.Register(MSG_ID_UALBERTA_SYS_STATUS, "MSG_ID_UALBERTA_SYS_STATUS", 3, 15, func(p mavlink.Packet) (mavlink.Message, error) {
