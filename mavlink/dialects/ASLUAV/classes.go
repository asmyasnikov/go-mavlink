...
type SensPower struct {
	Adc121VspbVolt float32 // [ V ]  Power board voltage sensor reading
	Adc121CspbAmp  float32 // [ A ]  Power board current sensor reading
	Adc121Cs1Amp   float32 // [ A ]  Board current sensor 1 reading
	Adc121Cs2Amp   float32 // [ A ]  Board current sensor 2 reading
}
...
type SensMppt struct {
	MpptTimestamp uint64  // [ us ]  MPPT last timestamp
	Mppt1Volt     float32 // [ V ]  MPPT1 voltage
	Mppt1Amp      float32 // [ A ]  MPPT1 current
	Mppt2Volt     float32 // [ V ]  MPPT2 voltage
	Mppt2Amp      float32 // [ A ]  MPPT2 current
	Mppt3Volt     float32 // [ V ] MPPT3 voltage
	Mppt3Amp      float32 // [ A ]  MPPT3 current
	Mppt1Pwm      uint16  // [ us ]  MPPT1 pwm
	Mppt2Pwm      uint16  // [ us ]  MPPT2 pwm
	Mppt3Pwm      uint16  // [ us ]  MPPT3 pwm
...
type AslctrlData struct {
	Timestamp       uint64  // [ us ]  Timestamp
	H               float32 //  See sourcecode for a description of these values...
	Href            float32 //
	HrefT           float32 //
	Pitchangle      float32 // [ deg ] Pitch angle
	Pitchangleref   float32 // [ deg ] Pitch angle reference
	Q               float32 //
	Qref            float32 //
	Uelev           float32 //
	Uthrot          float32 //
	Uthrot2         float32 //
	Nz              float32 //
	Airspeedref     float32 // [ m/s ] Airspeed reference
	Yawangle        float32 // [ deg ] Yaw angle
	Yawangleref     float32 // [ deg ] Yaw angle reference
	Rollangle       float32 // [ deg ] Roll angle
	Rollangleref    float32 // [ deg ] Roll angle reference
	P               float32 //
	Pref            float32 //
	R               float32 //
	Rref            float32 //
...
type EkfExt struct {
	...
	Windspeed float32 // [ m/s ]  Magnitude of wind velocity (in lateral inertial plane)
	Winddir   float32 // [ rad ]  Wind heading angle from North
	Windz     float32 // [ m/s ]  Z (Down) component of inertial wind velocity
	Airspeed  float32 // [ m/s ]  Magnitude of air velocity
	Beta      float32 // [ rad ]  Sideslip angle
	Alpha     float32 // [ rad ]  Angle of attack
}
...
type SensAtmos struct {
	Timestamp   uint64  // [ us ] Time since system boot
	Tempambient float32 // [ degC ]  Ambient temperature
...
type SensBatmon struct {
	BatmonTimestamp uint64  // [ us ] Time since system start
	Temperature     float32 // [ degC ] Battery pack temperature
	...
	Voltage         uint16  // [ mV ] Battery pack voltage
	Current         int16   // [ mA ] Battery pack current
	Batterystatus   uint16  // Battery monitor status report bits in Hex
	Serialnumber    uint16  // Battery monitor serial number in Hex
	Cellvoltage1    uint16  // [ mV ] Battery pack cell 1 voltage
	Cellvoltage2    uint16  // [ mV ] Battery pack cell 2 voltage
	Cellvoltage3    uint16  // [ mV ] Battery pack cell 3 voltage
	Cellvoltage4    uint16  // [ mV ] Battery pack cell 4 voltage
	Cellvoltage5    uint16  // [ mV ] Battery pack cell 5 voltage
	Cellvoltage6    uint16  // [ mV ] Battery pack cell 6 voltage
...
type FwSoaringData struct {
	Timestamp            uint64  // [ ms ] Timestamp
	Timestampmodechanged uint64  // [ ms ] Timestamp since last mode change
	Xw                   float32 // [ m/s ] Thermal core updraft strength
	Xr                   float32 // [ m ] Thermal radius
	Xlat                 float32 // [ deg ] Thermal center latitude
	Xlon                 float32 // [ deg ] Thermal center longitude
	Varw                 float32 // Variance W
	Varr                 float32 // Variance R
	Varlat               float32 // Variance Lat
	Varlon               float32 // Variance Lon
	Loiterradius         float32 // [ m ] Suggested loiter radius
	Loiterdirection      float32 // Suggested loiter direction
	Disttosoarpoint      float32 // [ m ] Distance to soar point
	Vsinkexp             float32 // [ m/s ] Expected sink rate at current airspeed, roll and throttle
	Z1Localupdraftspeed  float32 // [ m/s ] Measurement / updraft speed at current/local airplane position
	Z2Deltaroll          float32 // [ deg ] Measurement / roll angle tracking error
	Z1Exp                float32 // Expected measurement 1
	Z2Exp                float32 // Expected measurement 2
	Thermalgsnorth       float32 // [ m/s ] Thermal drift (from estimator prediction step only)
	Thermalgseast        float32 // [ m/s ] Thermal drift (from estimator prediction step only)
	TseDot               float32 // [ m/s ]  Total specific energy change (filtered)
...
type SensorpodStatus struct {
	Timestamp           uint64 // [ ms ] Timestamp in linuxtime (since 1.1.1970)
	...
	CPUTemp             uint8  // [ degC ] Temperature of sensorpod CPU in
}
...
type SensPowerBoard struct {
	Timestamp         uint64  // [ us ] Timestamp
	PwrBrdSystemVolt  float32 // [ V ] Power board system voltage
	PwrBrdServoVolt   float32 // [ V ] Power board servo voltage
	PwrBrdDigitalVolt float32 // [ V ] Power board digital voltage
	PwrBrdMotLAmp     float32 // [ A ] Power board left motor current sensor
	PwrBrdMotRAmp     float32 // [ A ] Power board right motor current sensor
	PwrBrdAnalogAmp   float32 // [ A ] Power board analog current sensor
	PwrBrdDigitalAmp  float32 // [ A ] Power board digital current sensor
	PwrBrdExtAmp      float32 // [ A ] Power board extension current sensor
	PwrBrdAuxAmp      float32 // [ A ] Power board aux current sensor
...
type GsmLinkStatus struct {
	Timestamp    uint64         // [ us ] Timestamp (of OBC)
...
type SatcomLinkStatus struct {
	Timestamp          uint64 // [ us ] Timestamp
	LastHeartbeat      uint64 // [ us ] Timestamp of the last successful sbd session
...
type SensorAirflowAngles struct {
	Timestamp          uint64  // [ us ] Timestamp
	Angleofattack      float32 // [ deg ] Angle of attack
	Sideslip           float32 // [ deg ] Sideslip angle
