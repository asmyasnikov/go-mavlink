...
type MountConfigure struct {
	TargetSystem    uint8          // System ID.
	TargetComponent uint8          // Component ID.
	MountMode       MAV_MOUNT_MODE // Mount operating mode.
...
type LimitsStatus struct {
	...
	ModsRequired  LIMIT_MODULE // AP_Limit_Module bitfield of required modules.
	ModsTriggered LIMIT_MODULE // AP_Limit_Module bitfield of triggered modules.
}
...
type RallyPoint struct {
	...
	Count           uint8       // Total number of points (for sanity checking).
	Flags           RALLY_FLAGS // Configuration flags.
}
...
type CameraStatus struct {
	...
	EventID      CAMERA_STATUS_TYPES // Event type.
}
...
type CameraFeedback struct {
	...
	TargetSystem uint8                 // System ID.
	CamIdx       uint8                 // Camera ID.
	Flags        CAMERA_FEEDBACK_FLAGS // Feedback flags.
}
...
type RemoteLogDataBlock struct {
	Seqno           MAV_REMOTE_LOG_DATA_BLOCK_COMMANDS // Log data block sequence number.
	TargetSystem    uint8                              // System ID.
	TargetComponent uint8                              // Component ID.
	Data            []uint8                            `len:"200" ` // Log data block.
}
...
type RemoteLogBlockStatus struct {
	...
	TargetSystem    uint8                              // System ID.
	TargetComponent uint8                              // Component ID.
	Status          MAV_REMOTE_LOG_DATA_BLOCK_STATUSES // Log data block status.
}
...
type LedControl struct {
	...
	CustomBytes     []uint8 `len:"24" ` // Custom Bytes.
}
...
type GoproHeartbeat struct {
	Status      GOPRO_HEARTBEAT_STATUS // Status.
	CaptureMode GOPRO_CAPTURE_MODE     // Current capture mode.
	Flags       GOPRO_HEARTBEAT_FLAGS  // Additional status bits.
}
...
type GoproGetRequest struct {
	TargetSystem    uint8         // System ID.
	TargetComponent uint8         // Component ID.
	CmdID           GOPRO_COMMAND // Command ID.
}
...
type GoproGetResponse struct {
	CmdID  GOPRO_COMMAND        // Command ID.
	Status GOPRO_REQUEST_STATUS // Status.
	Value  []uint8              `len:"4" ` // Value.
}
