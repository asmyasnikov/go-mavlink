...
type AqEscTelemetry struct {
	TimeBootMs  uint32   // Timestamp of the component clock since boot time in ms.
	Data0       []uint32 `len:"4" ` // Data bits 1-32 for each ESC.
	Data1       []uint32 `len:"4" ` // Data bits 33-64 for each ESC.
	StatusAge   []uint16 `len:"4" ` // Age of each ESC telemetry reading in ms compared to boot time. A value of 0xFFFF means timeout/no data.
	Seq         uint8    // Sequence number of message (first set of 4 motors is #1, next 4 is #2, etc).
	NumMotors   uint8    // Total number of active ESCs/motors on the system.
	NumInSeq    uint8    // Number of active ESCs in this sequence (1 through this many array members will be populated with data)
	Escid       []uint8  `len:"4" ` // ESC/Motor ID
	DataVersion []uint8  `len:"4" ` // Version of data structure (determines contents).
}
