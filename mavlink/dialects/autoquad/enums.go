...
// AUTOQUAD_MAVLINK_DEFS_VERSION type. Track current version of these definitions (can be used by checking value of AUTOQUAD_MAVLINK_DEFS_VERSION_ENUM_END). Append a new entry for each published change.
type AUTOQUAD_MAVLINK_DEFS_VERSION int

// MarshalBinary generic func
func (e AUTOQUAD_MAVLINK_DEFS_VERSION) MarshalBinary() (data []byte, err error) {
	return []byte(strconv.Itoa(int(e))), nil
}

// UnmarshalBinary generic func
func (e *AUTOQUAD_MAVLINK_DEFS_VERSION) UnmarshalBinary(data []byte) error {
	v, err := strconv.Atoi(string(data))
	if err != nil {
		return err
	}
	*e = AUTOQUAD_MAVLINK_DEFS_VERSION(v)
	return nil
}

const (
	// AQ_MAVLINK_DEFS_VERSION_1 enum
	AQ_MAVLINK_DEFS_VERSION_1 AUTOQUAD_MAVLINK_DEFS_VERSION = 1
)

func (e AUTOQUAD_MAVLINK_DEFS_VERSION) String() string {
	if name, ok := map[AUTOQUAD_MAVLINK_DEFS_VERSION]string{
		AQ_MAVLINK_DEFS_VERSION_1: "AQ_MAVLINK_DEFS_VERSION_1",
	}[e]; ok {
		return name
	}
	return fmt.Sprintf("AUTOQUAD_MAVLINK_DEFS_VERSION_UNDEFINED_%d", int(e))
}

// Bitmask return string representetion of intersects AUTOQUAD_MAVLINK_DEFS_VERSION enums
func (e AUTOQUAD_MAVLINK_DEFS_VERSION) Bitmask() string {
	bitmap := ""
	for _, entry := range []AUTOQUAD_MAVLINK_DEFS_VERSION{
...
// AUTOQUAD_NAV_STATUS type. Available operating modes/statuses for AutoQuad flight controller.  				Bitmask up to 32 bits. Low side bits for base modes, high side for  				additional active features/modifiers/constraints.
type AUTOQUAD_NAV_STATUS int

// MarshalBinary generic func
func (e AUTOQUAD_NAV_STATUS) MarshalBinary() (data []byte, err error) {
	return []byte(strconv.Itoa(int(e))), nil
}

// UnmarshalBinary generic func
func (e *AUTOQUAD_NAV_STATUS) UnmarshalBinary(data []byte) error {
	v, err := strconv.Atoi(string(data))
	if err != nil {
		return err
	}
	*e = AUTOQUAD_NAV_STATUS(v)
	return nil
}

const (
	// AQ_NAV_STATUS_INIT enum. System is initializing
	AQ_NAV_STATUS_INIT AUTOQUAD_NAV_STATUS = 0
	// AQ_NAV_STATUS_STANDBY enum. System is *armed* and standing by, with no throttle input and no autonomous mode
	AQ_NAV_STATUS_STANDBY AUTOQUAD_NAV_STATUS = 0x00000001
	// AQ_NAV_STATUS_MANUAL enum. Flying (throttle input detected), assumed under manual control unless other mode bits are set
	AQ_NAV_STATUS_MANUAL AUTOQUAD_NAV_STATUS = 0x00000002
	// AQ_NAV_STATUS_ALTHOLD enum. Altitude hold engaged
	AQ_NAV_STATUS_ALTHOLD AUTOQUAD_NAV_STATUS = 0x00000004
	// AQ_NAV_STATUS_POSHOLD enum. Position hold engaged
	AQ_NAV_STATUS_POSHOLD AUTOQUAD_NAV_STATUS = 0x00000008
	// AQ_NAV_STATUS_GUIDED enum. Externally-guided (eg. GCS) navigation mode
	AQ_NAV_STATUS_GUIDED AUTOQUAD_NAV_STATUS = 0x00000010
	// AQ_NAV_STATUS_MISSION enum. Autonomous mission execution mode
	AQ_NAV_STATUS_MISSION AUTOQUAD_NAV_STATUS = 0x00000020
	// AQ_NAV_STATUS_READY enum. Ready but *not armed*
	AQ_NAV_STATUS_READY AUTOQUAD_NAV_STATUS = 0x00000100
	// AQ_NAV_STATUS_CALIBRATING enum. Calibration mode active
	AQ_NAV_STATUS_CALIBRATING AUTOQUAD_NAV_STATUS = 0x00000200
	// AQ_NAV_STATUS_NO_RC enum. No valid control input (eg. no radio link)
	AQ_NAV_STATUS_NO_RC AUTOQUAD_NAV_STATUS = 0x00001000
	// AQ_NAV_STATUS_FUEL_LOW enum. Battery is low (stage 1 warning)
	AQ_NAV_STATUS_FUEL_LOW AUTOQUAD_NAV_STATUS = 0x00002000
	// AQ_NAV_STATUS_FUEL_CRITICAL enum. Battery is depleted (stage 2 warning)
	AQ_NAV_STATUS_FUEL_CRITICAL AUTOQUAD_NAV_STATUS = 0x00004000
	// AQ_NAV_STATUS_DVH enum. Dynamic Velocity Hold is active (PH with proportional manual direction override)
	AQ_NAV_STATUS_DVH AUTOQUAD_NAV_STATUS = 0x01000000
	// AQ_NAV_STATUS_DAO enum. Dynamic Altitude Override is active (AH with proportional manual adjustment)
	AQ_NAV_STATUS_DAO AUTOQUAD_NAV_STATUS = 0x02000000
	// AQ_NAV_STATUS_CEILING_REACHED enum. Craft is at ceiling altitude
	AQ_NAV_STATUS_CEILING_REACHED AUTOQUAD_NAV_STATUS = 0x04000000
	// AQ_NAV_STATUS_CEILING enum. Ceiling altitude is set
	AQ_NAV_STATUS_CEILING AUTOQUAD_NAV_STATUS = 0x08000000
	// AQ_NAV_STATUS_HF_DYNAMIC enum. Heading-Free dynamic mode active
	AQ_NAV_STATUS_HF_DYNAMIC AUTOQUAD_NAV_STATUS = 0x10000000
	// AQ_NAV_STATUS_HF_LOCKED enum. Heading-Free locked mode active
	AQ_NAV_STATUS_HF_LOCKED AUTOQUAD_NAV_STATUS = 0x20000000
	// AQ_NAV_STATUS_RTH enum. Automatic Return to Home is active
	AQ_NAV_STATUS_RTH AUTOQUAD_NAV_STATUS = 0x40000000
	// AQ_NAV_STATUS_FAILSAFE enum. System is in failsafe recovery mode
	AQ_NAV_STATUS_FAILSAFE AUTOQUAD_NAV_STATUS = 0x80000000
)

func (e AUTOQUAD_NAV_STATUS) String() string {
	if name, ok := map[AUTOQUAD_NAV_STATUS]string{
		AQ_NAV_STATUS_INIT:            "AQ_NAV_STATUS_INIT",
		AQ_NAV_STATUS_STANDBY:         "AQ_NAV_STATUS_STANDBY",
		AQ_NAV_STATUS_MANUAL:          "AQ_NAV_STATUS_MANUAL",
		AQ_NAV_STATUS_ALTHOLD:         "AQ_NAV_STATUS_ALTHOLD",
		AQ_NAV_STATUS_POSHOLD:         "AQ_NAV_STATUS_POSHOLD",
		AQ_NAV_STATUS_GUIDED:          "AQ_NAV_STATUS_GUIDED",
		AQ_NAV_STATUS_MISSION:         "AQ_NAV_STATUS_MISSION",
		AQ_NAV_STATUS_READY:           "AQ_NAV_STATUS_READY",
		AQ_NAV_STATUS_CALIBRATING:     "AQ_NAV_STATUS_CALIBRATING",
		AQ_NAV_STATUS_NO_RC:           "AQ_NAV_STATUS_NO_RC",
		AQ_NAV_STATUS_FUEL_LOW:        "AQ_NAV_STATUS_FUEL_LOW",
		AQ_NAV_STATUS_FUEL_CRITICAL:   "AQ_NAV_STATUS_FUEL_CRITICAL",
		AQ_NAV_STATUS_DVH:             "AQ_NAV_STATUS_DVH",
		AQ_NAV_STATUS_DAO:             "AQ_NAV_STATUS_DAO",
		AQ_NAV_STATUS_CEILING_REACHED: "AQ_NAV_STATUS_CEILING_REACHED",
		AQ_NAV_STATUS_CEILING:         "AQ_NAV_STATUS_CEILING",
		AQ_NAV_STATUS_HF_DYNAMIC:      "AQ_NAV_STATUS_HF_DYNAMIC",
		AQ_NAV_STATUS_HF_LOCKED:       "AQ_NAV_STATUS_HF_LOCKED",
		AQ_NAV_STATUS_RTH:             "AQ_NAV_STATUS_RTH",
		AQ_NAV_STATUS_FAILSAFE:        "AQ_NAV_STATUS_FAILSAFE",
	}[e]; ok {
		return name
	}
	return fmt.Sprintf("AUTOQUAD_NAV_STATUS_UNDEFINED_%d", int(e))
}

// Bitmask return string representetion of intersects AUTOQUAD_NAV_STATUS enums
func (e AUTOQUAD_NAV_STATUS) Bitmask() string {
	bitmap := ""
	for _, entry := range []AUTOQUAD_NAV_STATUS{
		AQ_NAV_STATUS_INIT,
...
const (
	// MAV_CMD_AQ_NAV_LEG_ORBIT enum. Orbit a waypoint. Params: 1) Orbit radius in meters; 2) Loiter time in decimal seconds; 3) Maximum horizontal speed in m/s; 4) Desired yaw angle at waypoint; 5) Latitude; 6) Longitude; 7) Altitude;
	MAV_CMD_AQ_NAV_LEG_ORBIT MAV_CMD = 1
	// MAV_CMD_AQ_TELEMETRY enum. Start/stop AutoQuad telemetry values stream. Params: 1) Start or stop (1 or 0); 2) Stream frequency in us; 3) Dataset ID (refer to aq_mavlink.h::mavlinkCustomDataSets enum in AQ flight controller code); 4) Empty; 5) Empty; 6) Empty; 7) Empty;
	MAV_CMD_AQ_TELEMETRY MAV_CMD = 2
	// MAV_CMD_AQ_REQUEST_VERSION enum. Request AutoQuad firmware version number. Params: 1) Empty; 2) Empty; 3) Empty; 4) Empty; 5) Empty; 6) Empty; 7) Empty;
...
func (e MAV_CMD) String() string {
	if name, ok := map[MAV_CMD]string{
		MAV_CMD_AQ_NAV_LEG_ORBIT:                   "MAV_CMD_AQ_NAV_LEG_ORBIT",
		MAV_CMD_AQ_TELEMETRY:                       "MAV_CMD_AQ_TELEMETRY",
		MAV_CMD_AQ_REQUEST_VERSION:                 "MAV_CMD_AQ_REQUEST_VERSION",
...
const (
	// MAV_DATA_STREAM_PROPULSION enum. Motor/ESC telemetry data
...
func (e MAV_DATA_STREAM) String() string {
	if name, ok := map[MAV_DATA_STREAM]string{
		MAV_DATA_STREAM_PROPULSION:      "MAV_DATA_STREAM_PROPULSION",
