...
func (e UALBERTA_AUTOPILOT_MODE) String() string {
	if name, ok := map[UALBERTA_AUTOPILOT_MODE]string{MODE_MANUAL_DIRECT: "MODE_MANUAL_DIRECT", MODE_MANUAL_SCALED: "MODE_MANUAL_SCALED", MODE_AUTO_PID_ATT: "MODE_AUTO_PID_ATT", MODE_AUTO_PID_VEL: "MODE_AUTO_PID_VEL", MODE_AUTO_PID_POS: "MODE_AUTO_PID_POS"}[e]; ok {
		return name
	}
	return fmt.Sprintf("UALBERTA_AUTOPILOT_MODE_ENUM_UNDEFINED_%d", int(e))
}
...
func (e UALBERTA_AUTOPILOT_MODE) Bitmask() string {
	bitmap := ""
	for _, entry := range []UALBERTA_AUTOPILOT_MODE{MODE_MANUAL_DIRECT, MODE_MANUAL_SCALED, MODE_AUTO_PID_ATT, MODE_AUTO_PID_VEL, MODE_AUTO_PID_POS} {
		if e&entry > 0 {
			if len(bitmap) > 0 {
				bitmap += " | "
			}
...
func (e UALBERTA_NAV_MODE) String() string {
	if name, ok := map[UALBERTA_NAV_MODE]string{NAV_AHRS_INIT: "NAV_AHRS_INIT", NAV_AHRS: "NAV_AHRS", NAV_INS_GPS_INIT: "NAV_INS_GPS_INIT", NAV_INS_GPS: "NAV_INS_GPS"}[e]; ok {
		return name
	}
	return fmt.Sprintf("UALBERTA_NAV_MODE_ENUM_UNDEFINED_%d", int(e))
}

// Bitmask return string representetion of intersects UALBERTA_NAV_MODE enums
func (e UALBERTA_NAV_MODE) Bitmask() string {
	bitmap := ""
	for _, entry := range []UALBERTA_NAV_MODE{NAV_AHRS_INIT, NAV_AHRS, NAV_INS_GPS_INIT, NAV_INS_GPS} {
		if e&entry > 0 {
			if len(bitmap) > 0 {
				bitmap += " | "
			}
...
func (e UALBERTA_PILOT_MODE) String() string {
	if name, ok := map[UALBERTA_PILOT_MODE]string{PILOT_MANUAL: "PILOT_MANUAL", PILOT_AUTO: "PILOT_AUTO", PILOT_ROTO: "PILOT_ROTO"}[e]; ok {
		return name
	}
	return fmt.Sprintf("UALBERTA_PILOT_MODE_ENUM_UNDEFINED_%d", int(e))
}

// Bitmask return string representetion of intersects UALBERTA_PILOT_MODE enums
func (e UALBERTA_PILOT_MODE) Bitmask() string {
	bitmap := ""
	for _, entry := range []UALBERTA_PILOT_MODE{PILOT_MANUAL, PILOT_AUTO, PILOT_ROTO} {
		if e&entry > 0 {
			if len(bitmap) > 0 {
				bitmap += " | "
			}
